...
FGraphNode SWorkflowGraphEditor::CreateUESourceNode(const FString& UEClassType, const FString& Id, FVector2D Position) const
{
	...
	else if (UEClassType == UEMeshyImportClassType)
	{
		...
		Node.WidgetValues.Add(TEXT("asset_path"), TEXT("/Game/UE_Gen/MeshyModels"));
		FComfyInputDef PathDef;
		PathDef.Name = TEXT("asset_path");
		PathDef.Type = TEXT("STRING");
		PathDef.DefaultString = TEXT("/Game/UE_Gen/MeshyModels");
	...
	else if (UEClassType == UESave3DModelClassType)
	{
		...
		// Widget: asset_path — content browser destination path
		Node.WidgetValues.Add(TEXT("asset_path"), TEXT("/Game/UE_Gen/Models"));
		FComfyInputDef PathDef;
		PathDef.Name = TEXT("asset_path");
		PathDef.Type = TEXT("STRING");
		PathDef.DefaultString = TEXT("/Game/UE_Gen/Models");
		Node.WidgetInputDefs.Add(TEXT("asset_path"), PathDef);
		Node.WidgetOrder.Add(TEXT("asset_path"));
	...
	else if (UEClassType == UEImageBridgeClassType)
	{
		...
		Node.WidgetValues.Add(TEXT("filename_prefix"), TEXT("UE_Gen_Bridge"));
		FComfyInputDef PrefixDef;
		PrefixDef.Name = TEXT("filename_prefix");
		PrefixDef.Type = TEXT("STRING");
		PrefixDef.DefaultString = TEXT("UE_Gen_Bridge");
	...
	else if (UEClassType == UEImageUpresClassType)
	{
		...
		Node.WidgetValues.Add(TEXT("filename_prefix"), TEXT("UE_Gen_Upres"));
		FComfyInputDef PrefixDef;
		PrefixDef.Name = TEXT("filename_prefix");
		PrefixDef.Type = TEXT("STRING");
		PrefixDef.DefaultString = TEXT("UE_Gen_Upres");
		Node.WidgetInputDefs.Add(TEXT("filename_prefix"), PrefixDef);
		Node.WidgetOrder.Add(TEXT("filename_prefix"));
...
void SWorkflowGraphEditor::SetExecutingNode(const FString& NodeId)
{
	...
	if (bChanged)
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: SetExecutingNode('%s') — highlighting updated"), *NodeId);
	...
	bool* OutNeedsDepth, FString* OutCameraDescription, bool* OutNeedsSegmentation) const
{
	...
	for (const FGraphNode& Node : Nodes)
	{
		...
		if (Node.ClassType == UEImageBridgeClassType)
		{
			...
			FString Prefix = TEXT("UE_Gen_Bridge");
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Image Bridge '%s' → SaveImage (prefix='%s'), downstream bypasses to node '%s' pin %d"),
				...
				if (Conn.TargetNodeId == Node.Id && Conn.TargetInputName == TEXT("mesh"))
				{
					...
					for (const FProxyCandidate& Candidate : Candidates)
					{
						...
						if (LinkInputs.Num() > 0)
						{
							...
							UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Selected '%s' as proxy (input='%s', saves_to_disk=%s)"),
					...
					if (!ProxyClassType.IsEmpty())
					{
						...
						ProxyMeta->SetStringField(TEXT("title"), TEXT("UE_Gen 3D Model Capture"));
						...
						{
							ProxyInputs->SetStringField(TEXT("filename_prefix"), TEXT("mesh/UE_Gen_Model"));
						}

						ProxyObj->SetObjectField(TEXT("inputs"), ProxyInputs);
						Workflow->SetObjectField(ProxyId, ProxyObj);

						UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Injected '%s' proxy (id=%s, saves_to_disk=%s) to capture model output from node '%s' pin %d"),
							*ProxyClassType, *ProxyId, bProxySavesToDisk ? TEXT("yes") : TEXT("no"), *RealSourceId, RealOutputIndex);
					}
					else
					{
						UE_LOG(LogTemp, Warning, TEXT("UE_Gen Export: No SaveGLB, Save3DModel, or ShowAny_UTK node available in ComfyUI to capture 3D model output. "
			...
			for (const FGraphConnection& Conn : Connections)
			{
				if (Conn.TargetNodeId == Node.Id && Conn.TargetInputName == TEXT("mesh"))
				{
					...
					if (!ProxyClassType2.IsEmpty())
					{
						...
						ProxyMeta2->SetStringField(TEXT("title"), TEXT("UE_Gen 3D Asset Export Capture"));
						...
						{
							ProxyInputs2->SetStringField(TEXT("filename_prefix"), TEXT("mesh/UE_Gen_Export"));
						}
						ProxyObj2->SetObjectField(TEXT("inputs"), ProxyInputs2);
						Workflow->SetObjectField(ProxyId2, ProxyObj2);

						UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Injected '%s' proxy (id=%s) for 3D Asset Export from node '%s' pin %d"),
		...
		// export are handled UE-side in SUE_GenPanel::ExecuteImageUpresExport()
		...
		if (Node.ClassType == UEImageUpresClassType)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Image Upres '%s' → ImageScaleBy (method=%s, scale=%.1f) "
		...
		if (Node.ClassType == TEXT("MeshyImageToModelNode"))
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Ensured all MeshyImageToModelNode V3 conditional inputs (id=%s)"), *Node.Id);
		}
			...
			if (Pin.Type.StartsWith(TEXT("COMFY_")) && !InputsObj->HasField(Pin.Name))
			{
				...
				if (!bHasConnection)
				{
					...
					else
					{
						...
						UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Emitting default 'true' for missing V3 input '%s' on node '%s'"),
			...
			for (const FComfyInputDef& DefInput : ExportDef->Inputs)
			{
				if (!DefInput.IsLinkType() && !Node.WidgetValues.Contains(DefInput.Name))
				{
					...
					if (!InputsObj->HasField(DefInput.Name))
					{
						...
						UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Emitting DB default for missing widget '%s' on node '%s'"),
		...
		for (auto& WfPair : Workflow->Values)
		{
			...
			for (const auto& InputPair : (*InputsObj)->Values)
			{
				...
				if (Bypass)
				{
					...
					UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Bridge bypass — node '%s' input '%s': [%s, *] → [%s, %d]"),
	...
	if (AdherenceOverride >= 0.0f)
	{
		...
		for (auto& WfPair : Workflow->Values)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Export: Prompt Adherence override (%.0f%%) → node '%s': cfg=%.1f, steps=%d, denoise=%.2f"),
...
FString SWorkflowGraphEditor::GetMeshySourceImageFilename() const
{
	...
	for (const FGraphNode& Node : Nodes)
	{
		if (Node.Id == SourceNodeId)
		{
			...
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: Meshy source image resolved to '%s' from node '%s'"),
