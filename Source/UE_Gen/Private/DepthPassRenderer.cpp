...
bool FDepthPassRenderer::CaptureDepth(float MaxDepthDistance, int32 DesiredWidth, int32 DesiredHeight)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: No active level viewport found"));
		return false;
	}
	...
	if (!SceneViewport)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: Could not get scene viewport"));
		return false;
	}
	...
	if (!World)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: No editor world available"));
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Camera at (%.1f, %.1f, %.1f) Rot (%.1f, %.1f, %.1f) FOV %.1f"),
	...
	if (!CaptureActor)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: Failed to spawn capture actor"));
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: CaptureComp world location: (%.1f, %.1f, %.1f)"),
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: Failed to read depth render target"));
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Captured scene depth %dx%d (max depth: %.0f UU)"),
...
TArray<FColor> FDepthPassRenderer::ReadAndConvertDepthRT(UTextureRenderTarget2D* RT, float MaxDepthDistance)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: RT or RT resource is null"));
	...
	if (!RTResource)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: RTResource is null"));
		return GrayscalePixels;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: ReadLinearColorPixels returned %d pixels (expected %d)"),
	...
	if (LinearPixels.Num() != Width * Height)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: ReadFloat16Pixels returned %d pixels, success=%d"),
			FloatPixels.Num(), bReadSuccess ? 1 : 0);

		if (!bReadSuccess || FloatPixels.Num() != Width * Height)
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: Both pixel read methods failed"));
			return GrayscalePixels;
		}
	...
	auto LogSample = [&](const TCHAR* Label, int32 X, int32 Y)
	{
		if (X >= 0 && X < Width && Y >= 0 && Y < Height)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth Sample [%s] (%d,%d): R=%.4f G=%.4f B=%.4f A=%.4f"),
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Pixel stats - Zero: %d, Negative: %d, Positive: %d, Inf/NaN: %d, Total: %d"),
		ZeroCount, NegativeCount, PositiveCount, InfCount, RawDepths.Num());

	if (PositiveCount > 0)
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Average positive depth: %.2f"), SumPositive / PositiveCount);
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Depth: No valid depth pixels found"));
		return GrayscalePixels;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Valid pixels: %d, Percentile range: %.1f - %.1f UU (of raw %.1f - %.1f)"),
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Depth: Output - Black: %d, White: %d, Mid-gray: %d"),
