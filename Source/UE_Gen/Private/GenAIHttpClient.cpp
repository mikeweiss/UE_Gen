...
void FGenAIHttpClient::FetchAvailableModels()
{
		...
		[this](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to fetch /object_info from ComfyUI"));
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to parse /object_info JSON"));
			OnModelListsFetched.ExecuteIfBound(Checkpoints, LoRAs, ControlNets, GeminiOpts, KlingOpts);
			return;
		}
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Discovered %d checkpoints, %d LoRAs, %d ControlNets, %d Gemini models, %d Kling models, %d KlingVideo models, %d Veo3 models, %d Wan models"),
	...
	const TArray<FLoRAEntry>& ActiveLoRAs)
{
	...
	if (Settings->bEnableDepthControlNet && !DepthImageBase64.IsEmpty())
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to upload depth image, proceeding without ControlNet"));
		}
	}
...
void FGenAIHttpClient::SendGeminiRequest(
	...
	const FString& Prompt)
{
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: SendGeminiRequest called (bRequestInProgress=%s, CurrentPromptId='%s')"),
	...
	if (!ViewportImageBase64.IsEmpty())
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to upload viewport image for Gemini, proceeding without it"));
		}
	}
	...
	if (!DepthImageBase64.IsEmpty())
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to upload depth image for Gemini, proceeding without it"));
		}
	}
...
void FGenAIHttpClient::SendKlingRequest(
	...
	const FString& NegativePrompt)
{
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: SendKlingRequest called (bRequestInProgress=%s, CurrentPromptId='%s')"),
		bRequestInProgress ? TEXT("true") : TEXT("false"), *CurrentPromptId);
	...
	if (!ViewportImageBase64.IsEmpty())
	{
		...
		if (!UploadImageToComfyUI(ViewportImageBase64, UploadName, ViewportFilename))
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to upload viewport image for Kling, proceeding without it"));
		}
	}
...
bool FGenAIHttpClient::UploadImageToComfyUI(const FString& Base64PNG, const FString& DesiredFilename, FString& OutServerFilename)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to decode base64 for upload"));
		return false;
	}
	...
	Request->OnProcessRequestComplete().BindLambda(
		[&bCompleted, &bSuccess, &ResponseBody](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen Upload: HTTP %d: %s"),
	...
	if (!bSuccess)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Image upload failed for %s"), *DesiredFilename);
		return false;
	}
	...
	if (FJsonSerializer::Deserialize(Reader, JsonResp) && JsonResp.IsValid())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Uploaded image as '%s'"), *OutServerFilename);
		return true;
	}
	...
	const TArray<FLoRAEntry>& ActiveLoRAs) const
{
	...
	// -- SaveImage --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen"));
	...
	const FString& Prompt) const
{
	...
	// -- SaveImage --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_Gemini"));
		Workflow->SetObjectField(SaveImageId, MakeNode(TEXT("SaveImage"), Inputs));
	}
	...
	const FString& NegativePrompt) const
{
	...
	// -- SaveImage --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_Kling"));
		Workflow->SetObjectField(SaveImageId, MakeNode(TEXT("SaveImage"), Inputs));
	}
	...
	const FString& NegativePrompt) const
{
	...
	// -- SaveVideo to save the VIDEO output --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_KlingVideo"));
	...
	const FString& Prompt) const
{
	...
	// -- SaveVideo to save the VIDEO output --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_Veo3"));
		Inputs->SetStringField(TEXT("format"), TEXT("auto"));
		Inputs->SetStringField(TEXT("codec"), TEXT("auto"));
		Workflow->SetObjectField(SaveVideoId, MakeNode(TEXT("SaveVideo"), Inputs));
	}
	...
	const FString& NegativePrompt) const
{
	...
	// -- SaveVideo to save the VIDEO output --
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_Wan"));
		Inputs->SetStringField(TEXT("format"), TEXT("auto"));
		Inputs->SetStringField(TEXT("codec"), TEXT("auto"));
		Workflow->SetObjectField(SaveVideoId, MakeNode(TEXT("SaveVideo"), Inputs));
	}
	...
	int32& NextNodeId) const
{
	...
	// SaveImage from hi-res output
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_HiRes"));
...
void FGenAIHttpClient::SubmitPrompt(TSharedPtr<FJsonObject> Workflow)
{
	...
	// DB dependency, so it works even if /object_info hasn't been fetched yet.
	{
		for (const auto& NodePair : Workflow->Values)
		{
			...
			if (ClassType == TEXT("MeshyImageToModelNode"))
			{
				...
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: Ensured all MeshyImageToModelNode V3 conditional inputs (id=%s)"), *NodePair.Key);
			}
		...
		if (DB.IsPopulated())
		{
			for (const auto& NodePair : Workflow->Values)
			{
				...
				for (const FComfyInputDef& DefInput : Def->Inputs)
				{
					...
					UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Injected missing input '%s' for node '%s' (id=%s)"),
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Submitting workflow JSON:\n%s"), *PayloadString);
...
void FGenAIHttpClient::OnPromptResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Prompt queued with ID: %s"), *CurrentPromptId);
...
void FGenAIHttpClient::OnHistoryResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	if ((*PromptResult)->TryGetObjectField(TEXT("status"), StatusObj))
	{
		...
		if ((*StatusObj)->TryGetStringField(TEXT("status_str"), StatusStr))
		{
			if (StatusStr == TEXT("error"))
			{
							...
							if (MsgType == TEXT("execution_error"))
							{
								...
								if (MsgData.IsValid())
								{
									...
									if (MsgData->TryGetArrayField(TEXT("traceback"), TBArray))
									{
										...
										UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Python traceback:\n%s"), *FullTB.Left(3000));
									}
								}
							}
						}
				...
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: %s (checking for partial outputs...)"), *ErrorDetail);
			}
		}
	}
	...
	if (!(*PromptResult)->TryGetObjectField(TEXT("outputs"), OutputsObj))
	{
		if (bHasError)
		{
			...
			PendingSegmentation.Reset();
			CurrentPromptId.Empty();
			UE_LOG(LogTemp, Error, TEXT("UE_Gen: %s"), *ErrorDetail);
	...
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Outputs object has %d node entries"), (*OutputsObj)->Values.Num());
		for (auto& Pair : (*OutputsObj)->Values)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Output node '%s': %s"), *Pair.Key, *ValStr.Left(2000));
		}
	}
	...
	if (bIsSAM3Request)
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen SAM3: History found for prompt %s — collecting outputs..."), *CurrentPromptId);
		PendingSegmentation = MakeShareable(new FPendingSegFetch());

		for (auto& Pair : (*OutputsObj)->Values)
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen SAM3: Output node '%s'"), *Pair.Key);
			...
			// Determine if this output node is the visualization (filename prefix "UE_Gen_SAM3_vis")
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen SAM3: Fetching %d output images"), PendingSegmentation->ImagesToFetch.Num());
		...
		for (const auto& Pair : (*OutputsObj)->Values)
		{
			...
			for (const auto& Field : (*NodeOutput)->Values)
			{
					...
					for (const auto& Item : *FileArray)
					{
						...
						if (Item->TryGetObject(FileObj))
						{
							...
							if (!Filename.IsEmpty() && IsMeshFilename(Filename))
							{
								...
								UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' field '%s' produced 3D model file object: %s (subfolder=%s, type=%s)"),
						...
						if (Item->TryGetString(StringVal) && !StringVal.IsEmpty() && IsMeshFilename(StringVal))
						{
							...
							UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' field '%s' produced 3D model string path: %s"),
				...
				if (Field.Value->TryGetString(StrVal) && !StrVal.IsEmpty() && IsMeshFilename(StrVal))
				{
					...
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' field '%s' has 3D model string: %s"),
				...
				if (Field.Value->TryGetObject(SingleObj))
				{
					...
					if (!Filename.IsEmpty() && IsMeshFilename(Filename))
					{
						...
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' field '%s' has 3D model object: %s (subfolder=%s, type=%s)"),
							*Pair.Key, *FieldKey, *Filename, *Last3DModelSubfolder, *Last3DModelFolderType);
						break;
					}
				}
			...
			for (const auto& Pair : (*OutputsObj)->Values)
			{
				...
				if ((*NodeOutput)->TryGetArrayField(TEXT("text"), TextArray) && TextArray->Num() > 0)
				{
					...
					if (IsMeshFilename(TextResult))
					{
						...
						bHas3DModelResult = true;
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' produced 3D model path in text: %s (type=%s)"),
					...
					{
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' produced File3D stream: %s — "
	...
	for (auto& Pair : (*OutputsObj)->Values)
	{
		...
		if ((*NodeOutput)->TryGetArrayField(TEXT("images"), ImagesArray) && ImagesArray->Num() > 0)
		{
			...
			if (ImageInfo.IsValid())
			{
				...
				{
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Skipping 3D model file '%s' found under 'images' key (node %s)"), *Filename, *Pair.Key);
					continue;
				}
				...
				if (IsVideoFile(Filename) && !bFoundVideo)
				{
					...
					bFoundVideo = true;
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Found video file '%s' under 'images' key (node %s) — treating as video"), *Filename, *Pair.Key);
					continue;
				}
				...
				if (Filename.StartsWith(TEXT("__UE_Upres_Export__")))
				{
					...
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Found upres output image: %s (node %s)"), *Filename, *Pair.Key);
				}
				else if (!bFoundFirstImage)
				{
					FirstImage = { Filename, Subfolder, FolderType };
					bFoundFirstImage = true;
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Found image output: %s (node %s)"), *Filename, *Pair.Key);
				}
			}
		}
	...
	if (bFoundUpresImage || bFoundFirstImage)
	{
		...
		if (bFoundUpresImage)
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Generation complete, fetching UPRES image: %s"), *Chosen.Filename);
		}
		else
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Generation complete, fetching image: %s"), *Chosen.Filename);
		}
		OnProgress.ExecuteIfBound(0.9f);
	...
	if (bFoundVideo)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Video generation complete%s: %s"),
	...
	if (bHas3DModelResult)
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: 3D model detected as sole workflow output, completing"));
		bRequestInProgress = false;
		CurrentPromptId.Empty();
		OnComplete.ExecuteIfBound(true, nullptr);
		return;
	...
	for (auto& Pair : (*OutputsObj)->Values)
	{
		...
		if ((*NodeOutput)->TryGetArrayField(TEXT("text"), TextArray) && TextArray->Num() > 0)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' produced text output: %s"), *Pair.Key, *TextResult.Left(500));
		...
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Node '%s' produced non-visual output (%d fields)"), *Pair.Key, (*NodeOutput)->Values.Num());
	...
	if (StatusObj)
	{
		...
		if (bCompleted)
		{
			...
			if (!bHasError)
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: Workflow completed with no visual outputs — signaling success"));
				OnComplete.ExecuteIfBound(true, nullptr);
			}
			else
			{
...
void FGenAIHttpClient::FetchResultImage(const FString& Filename, const FString& Subfolder, const FString& FolderType)
{
	...
	if (CurrentRequest.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: FetchResultImage skipped — a fetch is already in flight"));
		return;
	}
...
void FGenAIHttpClient::OnImageFetchComplete(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	bRequestInProgress = false;
	CurrentPromptId.Empty();
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: OnImageFetchComplete (connected=%s, bRequestInProgress now=false)"),
...
UTexture2D* FGenAIHttpClient::DecodeImageToTexture(const TArray<uint8>& ImageData)
{
	...
	if (!ImageWrapper.IsValid() || !ImageWrapper->SetCompressed(ImageData.GetData(), ImageData.Num()))
	{
		...
		if (!ImageWrapper.IsValid() || !ImageWrapper->SetCompressed(ImageData.GetData(), ImageData.Num()))
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Could not decode image (tried PNG and JPEG)"));
			return nullptr;
		}
	}
...
TSharedPtr<FJsonObject> FGenAIHttpClient::BuildSAM3Workflow(const FString& ImageFilename) const
{
	...
	// Node 4: SaveImage (save individual mask images — one per detected object)
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_SAM3_mask"));
	...
	// Node 5: SaveImage (save visualization — annotated image with boxes + scores)
	{
		...
		Inputs->SetStringField(TEXT("filename_prefix"), TEXT("UE_Gen_SAM3_vis"));
...
void FGenAIHttpClient::OnSegmentationImageFetchComplete(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	...
	if (bConnectedSuccessfully && Response.IsValid() && Response->GetResponseCode() == 200)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen SAM3: Failed to decode image %d"), Idx);
		}
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen SAM3: Failed to fetch image %d (HTTP error)"), Idx);
	}
...
void FGenAIHttpClient::OpenOutputFolder()
{
		...
		[this, Settings](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
	{
		...
		if (!OutputDir.IsEmpty())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Opening output folder: %s"), *OutputDir);
		...
		else
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Could not find output dir, opening in browser: %s"), *ViewURL);
...
bool FGenAIHttpClient::UploadRawFile(const TArray<uint8>& FileBytes, const FString& DesiredFilename, const FString& ContentType, FString& OutServerFilename)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[&bCompleted, &bSuccess, &ResponseBody](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen UploadRawFile: HTTP %d: %s"),
	...
	if (!bSuccess)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to upload file %s"), *DesiredFilename);
		return false;
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Could not parse upload response for %s"), *DesiredFilename);
	return false;
}
...
void FGenAIHttpClient::FetchImageThumbnail(const FString& ImageFilename, TFunction<void(UTexture2D*)> OnThumbnailReady)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[this, OnThumbnailReady, ImageFilename](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
		{
			if (!bConnected || !Resp.IsValid() || Resp->GetResponseCode() != 200)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to fetch thumbnail for '%s' (HTTP %d)"),
...
void FGenAIHttpClient::DownloadComfyUIImageAsBase64(const FString& ImageFilename, TFunction<void(const FString&)> OnBase64Ready)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[OnBase64Ready, ImageFilename](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
		{
			if (!bConnected || !Resp.IsValid() || Resp->GetResponseCode() != 200)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to download image '%s' from ComfyUI (HTTP %d)"),
					*ImageFilename, Resp.IsValid() ? Resp->GetResponseCode() : 0);
				if (OnBase64Ready) OnBase64Ready(FString());
				return;
			}
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloaded '%s' from ComfyUI (%d bytes, %d chars base64)"),
...
void FGenAIHttpClient::ConnectWebSocket()
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Connecting WebSocket to %s"), *WsURL);
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to create WebSocket"));
		return;
	}
	...
	WebSocket->OnConnected().AddLambda([this]()
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: WebSocket connected"));
	});

	WebSocket->OnConnectionError().AddLambda([this](const FString& Error)
	{
		bWebSocketConnected = false;
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: WebSocket connection error: %s"), *Error);
	});

	WebSocket->OnClosed().AddLambda([this](int32 StatusCode, const FString& Reason, bool bWasClean)
	{
		bWebSocketConnected = false;
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: WebSocket closed (code=%d, reason=%s)"), StatusCode, *Reason);
	});
...
void FGenAIHttpClient::OnWebSocketMessage(const FString& Message)
{
	...
	if (MsgType == TEXT("executing"))
	{
		...
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: WS executing node '%s' (prompt=%s)"), *NodeId, *PromptId);
			OnNodeExecuting.ExecuteIfBound(NodeId);
		}
		else
		{
			// "node" is null → execution finished for this prompt
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: WS execution complete (node=null, prompt=%s)"), *PromptId);
			...
			for (const auto& Field : (*OutputObj)->Values)
			{
					...
					for (const auto& Item : *Arr)
					{
						...
						if (Item->TryGetObject(FileObj))
						{
							...
							if (!Filename.IsEmpty() && IsMeshFilename(Filename))
							{
								...
								if (Last3DModelFolderType.IsEmpty()) Last3DModelFolderType = TEXT("output");
								bHas3DModelResult = true;
								UE_LOG(LogTemp, Log, TEXT("UE_Gen: WS executed node '%s' produced 3D model: %s (from output object)"), *NodeId, *Filename);
								break;
							}
						}
						FString StrVal;
						if (Item->TryGetString(StrVal) && !StrVal.IsEmpty() && IsMeshFilename(StrVal))
						{
							...
							Last3DModelFolderType = TEXT("output");
							bHas3DModelResult = true;
							UE_LOG(LogTemp, Log, TEXT("UE_Gen: WS executed node '%s' produced 3D model path: %s"), *NodeId, *StrVal);
							break;
						}
					}
				}
				...
				if (!bHas3DModelResult)
				{
					...
					if (Field.Value->TryGetString(StrVal) && !StrVal.IsEmpty() && IsMeshFilename(StrVal))
					{
						...
						Last3DModelFolderType = TEXT("output");
						bHas3DModelResult = true;
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: WS executed node '%s' field '%s' has 3D model: %s"), *NodeId, *Field.Key, *StrVal);
					}
				}
	...
	TFunction<void(const FString&)> OnDownloadComplete)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloading file from: %s"), *ViewURL);
	...
	Request->OnProcessRequestComplete().BindLambda(
		[OnDownloadComplete, Filename, LocalSavePath](FHttpRequestPtr Req, FHttpResponsePtr Resp, bool bConnected)
		{
			if (!bConnected || !Resp.IsValid() || Resp->GetResponseCode() != 200)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to download file '%s' from ComfyUI (HTTP %d)"),
			...
			if (RawBytes.Num() == 0)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Downloaded file '%s' is empty"), *Filename);
				if (OnDownloadComplete) OnDownloadComplete(FString());
				return;
			}
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloaded '%s': %d bytes, Content-Type: %s"),
				*Filename, RawBytes.Num(), *ContentType);

			// Log first bytes to help diagnose format issues (e.g. GLB should start with 'glTF')
			if (RawBytes.Num() >= 4)
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: First 4 bytes: 0x%02X 0x%02X 0x%02X 0x%02X ('%c%c%c%c')"),
			...
			if (ContentType.Contains(TEXT("text")) || ContentType.Contains(TEXT("json")))
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Downloaded file '%s' appears to be text, not binary: %s"),
			...
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloaded '%s' from ComfyUI (%d bytes) -> %s"),
					*Filename, RawBytes.Num(), *LocalSavePath);
				if (OnDownloadComplete) OnDownloadComplete(LocalSavePath);
			}
			else
			{
				UE_LOG(LogTemp, Error, TEXT("UE_Gen: Failed to save downloaded file to %s"), *LocalSavePath);
				if (OnDownloadComplete) OnDownloadComplete(FString());
			}
		});
