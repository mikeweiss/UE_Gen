...
bool FSegmentationCapture::CaptureSegmentation(const FString& Mode, int32 DesiredWidth, int32 DesiredHeight)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: No active level viewport found"));
		return false;
	}
	...
	if (!SceneViewport)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: Could not get scene viewport"));
		return false;
	}
	...
	if (!World)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: No editor world available"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: No visible actors found"));
		return false;
	}

	UE_LOG(LogTemp, Log, TEXT("UE_Gen Segmentation: Found %d visible actors to segment"), VisibleActors.Num());
	...
	if (!CaptureActor)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: Failed to spawn capture actor"));
		return false;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Segmentation: Captured %dx%d mask with %d segments from %d actors"),
...
bool FSegmentationCapture::SegmentGeneratedImage(UTexture2D* GeneratedTexture)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: No mask data or no generated texture"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Segmentation: Failed to read generated image pixels"));
		return false;
	}

	UE_LOG(LogTemp, Log, TEXT("UE_Gen Segmentation: Segmenting generated image %dx%d (mask %dx%d)"),
	...
	for (const FMaskRegion& Region : MaskRegions)
	{
		...
		if (bHadPixels)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen Segmentation: Isolated '%s' from generated image (%d bytes PNG)"),
				*SegmentedActors.Last().ActorName, SegmentedActors.Last().IsolationPNGData.Num());
		}
	}

	UE_LOG(LogTemp, Log, TEXT("UE_Gen Segmentation: Produced %d isolation images from generated result"),
		SegmentedActors.Num());
