...
void FMeshPreviewRenderer::SetupPreviewScene()
{
	...
	if (!World)
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen MeshPreview: No editor world available"));
		return;
	}
	...
	if (!NewActor)
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen MeshPreview: Failed to spawn preview actor"));
		return;
	}
...
void FMeshPreviewRenderer::Render()
{
		...
		!RT || !::IsValid(RT))
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("ViewGen MeshPreview: Scene objects invalidated by GC, will recreate"));
		bSceneReady = false;
		PreviewActor.Reset();
		MeshComponent.Reset();
		CaptureComponent.Reset();
...
UTexture2D* FMeshPreviewRenderer::CreateTextureFromRT(UTextureRenderTarget2D* RT)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen MeshPreview: Failed to read render target pixels"));
		return nullptr;
	}
