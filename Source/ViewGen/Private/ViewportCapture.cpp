...
bool FViewportCapture::CaptureActiveViewport(int32 DesiredWidth, int32 DesiredHeight)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen: No active level viewport found"));
		return false;
	}
	...
	if (!SceneViewport)
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen: Could not get scene viewport"));
		return false;
	}
	...
	FEditorViewportClient& ViewportClient = ActiveLevelViewport->GetLevelViewportClient();
	{
		...
		UE_LOG(LogTemp, Log, TEXT("ViewGen: Camera FOV=%.1f, ~%dmm equiv, Loc=(%s), Rot=(%s)"),
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen: Viewport has invalid size"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen: Failed to read viewport pixels"));
		return false;
	}
	...
	if (!CapturedTexture)
	{
		UE_LOG(LogTemp, Warning, TEXT("ViewGen: Failed to create texture from %dx%d viewport pixels (%d pixels)"),
			CaptureWidth, CaptureHeight, RawPixels.Num());
		return false;
	}
	CapturedTexture->AddToRoot();

	UE_LOG(LogTemp, Log, TEXT("ViewGen: Captured viewport %dx%d (%d bytes PNG)"),
